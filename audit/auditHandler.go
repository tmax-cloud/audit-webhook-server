...
func ListAuditResource(w http.ResponseWriter, r *http.Request) {
	util.SetResponse(w, "", util.AuditResourceList, http.StatusOK)
	return
}
...
func MemberSuggestions(res http.ResponseWriter, r *http.Request) {
	...
	if sarResult, err := util.CreateSubjectAccessReview(userId, nil, "", "namespaces", "", "", "list"); err != nil {
		klog.Errorln(err)
...
func GetAudit(res http.ResponseWriter, req *http.Request) {
	...
	// ns get줘도 감사기록은 안보이게..
	nsListSAR, err := util.CreateSubjectAccessReview(userId, userGroups, "", "namespaces", "", "", "list")
	if err != nil {
		klog.Errorln(err)
	...
	if !nsListSAR.Status.Allowed {
		...
		if nsList = util.GetAccessibleNS(userId, "", userGroups); len(nsList.Items) == 0 {
